...
<body>
  ...
  <main class="container-fluid text-center">
    <p>
      <?php 
      $dataPO = json_decode(file_get_contents('json_files/POsZ.json'));
      for ($i=0; $i < sizeof($dataPO); $i++) {  
        if ($_SESSION['client_id'] == $dataPO[$i]->clientIDZ) {
          echo "Purchase Order Number <b>" . $dataPO[$i]->poNoZ . "</b> is <b>" . $dataPO[$i]->statusZ . "</b> On this Date: <b>" . $dataPO[$i]->datePOZ . "</b><br><br>";
        }
      }
      ?>
    </p>
  ...
  if (isset($_POST['Submit'])) {
    $pos = $_POST['number'];
    $line = json_decode(file_get_contents('json_files/LinesZ.json'));
    $posArray = json_decode(file_get_contents('json_files/POsZ.json'));
    $alldata = [];
    for ($i=0; $i < sizeof($line); $i++) {  
      if($pos === $line[$i]->poNoZ){
        for ($j=0; $j < sizeof($posArray); $j++) { 
          if ($posArray[$j]->poNoZ === $pos && $posArray[$j]->clientIDZ ===  $Client_Data->clientIDZ) {
            $alldata[] = $line[$i];
          }
        }
      } 
